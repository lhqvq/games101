...
{   
    ...
    bp = p - _v[1];
    cp = p - _v[2];
    auto z1 = ab.cross(ap)[2], z2 = bc.cross(bp)[2], z3 = ca.cross(cp)[2];
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float left = width, right = 0, down = height, top = 0;
    for (int i = 0; i < 3; ++i) {
        left = std::min(left, v[i][0]);
        right = std::max(right, v[i][0]);
        down = std::min(down, v[i][1]);
    ...
    for (int x = left; x <= right; ++x) {
        for (int y = down; y <= top; ++y) {
            if (insideTriangle(x + 0.5, y + 0.5, t.v)) {
                ...
                Eigen::Vector3f cur_point = Eigen::Vector3f(x, y, z_interpolated);
